...
known1$order_date = unclass(as.Date(known1$order_date)); unknown1$order_date = unclass(as.Date(unknown1$order_date))
...
known1$delivery_date = unclass(as.Date(known1$delivery_date)); unknown1$delivery_date = unclass(as.Date(unknown1$delivery_date))
...
known1$yob_cat = addNA(known1$yob_cat); unknown1$yob_cat = addNA(unknown1$yob_cat)
#levels(known1$yob_cat); levels(unknown1$yob_cat)
...
known1$user_reg_date = unclass(as.Date(known1$user_reg_date)); unknown1$user_reg_date = unclass(as.Date(unknown1$user_reg_date))
...
## splitting known data into train and test set
# set.seed(12)
# n = nrow(known1)
...
set.seed(26)
known1$set_id = sample(c(1, 2, 3, 4), size = nrow(known1), replace = TRUE, prob = c(0.2, 0.2, 0.2, 0.4))
...
features1 = c(3, 4, 5, 7, 13, 16, 20, 23, 24, 27, 32, 34, 39)
features2 = c(2, 6, 8, 9, 10, 11, 12, 14, 21, 22, 23, 26, 30, 39)
features3 = c(17, 19, 23, 25, 29, 33, 35, 36, 37, 38, 39)

features = list(features1, features2, features3)

head(known1[, features2])
